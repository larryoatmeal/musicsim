...
void init(number *walls, number *excitor, number *beta, number *sigma){
    ...
    //beta
    for(int i = 0; i < SIMULATION_SIZE + PADDING; i++){
    ...
    if(n % 100 == 0){
      ...
      for(int y = 0; y < H; y++){
        for(int x = 0; x < W; x++){
          ...
          // number p_denom = (1 - beta[i] + sigma[i]);
          // number val = p_denom;
          number val = max(min(p[index_of_padded(x, y)] * 1000, 1.0f), -1.0f);

          if(val < 0){
            Vector3f color(0, -val, 0);
            image.setPixel(x, y, color);
          }else{
            Vector3f color(val, 0, 0);
            image.setPixel(x, y, color);
